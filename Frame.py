...
class Frame:
    ...
    def run_detection(self, model):
        ...
        player_id = 0 # ID number of the player to be used
    ...
    def bound_box_diff(self, frames):
        ...
        # Find the highest ID number used in this clip so far
        highest_id = self.get_highest_id(frames)

        # List containing the most recent instance of players with each ID
        most_rec_players = []
        found = False

        # Loop through each player ID in use to find a match
        for id in range(highest_id+1):
            found = False
            # Search through frames until match is found starting from the most recent
            for frame in frames[::-1]:
                if not found:
                    # For each player in the frame, check if the ID matches
                    for player in frame.player_list:
                        if player.id == id:
                            # Append the player to the list of most recent players
        ...
        # Loop through each player in the current frame that needs ID
        for cur_player in poss_play_ids:
            closest = 0
            smallest_diff = 100000
            # Check each player against the most recent players with IDs
            for past_player in most_rec_players:
                # Calculate the difference in bounding box position betweeen the two players
                start_diff, end_diff = cur_player.bb_diff(past_player)
                tot_diff = start_diff + end_diff
                # Find the smallest difference and infer that this is the same player
